...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            val bitmap = BitmapFactory.decodeFile("/sdcard/tmp/fruit.jpg")
    ...
    private fun startCompress() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            val test = File("/sdcard/tmp/fruit.jpg")
            val result = JpegUtils.getTempFile(this@MainActivity, "result", ".jpg")
            val ret = try {
                LibJpegTurbo.jpegTurboFile(test, result)
            } catch (e: Exception) {
                e.printStackTrace()
                false
            }
            val time = System.currentTimeMillis() - start
            val msg = "jpegTurboFile:$ret, before=${test.length()}, after=${result.length()}, time=$time"
